    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                '''
                request = self._requestQueue.get(True,5)
            except Queue.Empty:
                continue
            else:
                ...
                try:
                    ...
                    self._resultQueue.put((request,result,self))
                except:
                    '''异常处理'''
                    request.exception = True
                    self._resultQueue.put((request,sys.exc_info(),self))
    ...
    def poll(self,block = False):
        while True:
            ...
            try:
                ...
                request , result ,worker = self._resultQueue.get(block=block)
                if request.exception and request.exc_callback:
                    request.exc_callback(request,result)
                if request.callback and not (request.exception and request.exc_callback):
                    request.callback(request,result,worker)

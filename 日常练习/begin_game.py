...
def parseArgs():
    ...
    optparse.add_option("-n","--notify-state",action="store_false",help="不开启报警".decode("utf-8"))
    optparse.add_option("-a","--start-all",action="store_true",help="开启所有区服".decode("utf-8"))
    optparse.add_option('-s','--server-id',dest="server_id",help="指定开启区服的ID,有-a参数时此参数无效".decode("utf-8"))
...
def main():
    ...
    if options.start_all is None and options.server_id is None:
        server_id = get_server_id(opts['grains'].get('server_list',None))
    params = {
        "notify_state": options.notify_state,
        "start_all" : options.start_all,
        "server_id": server_id
    }
    ret = minion['uqee.start_game'](**params)
